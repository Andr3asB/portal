...
| Sportradar-Embeds (`embed-api.eui.connect.sportradar.com/v1/embed/<n>/…`) | `18_tvb.py` (Spiele und Tabelle der Profis) | keine | live je Seitenaufruf | **Zwei Embeds: 248 = Opel HBL, 255 = DHB-Pokal** (#151 wäre sonst beim Neubau lautlos rückgängig gemacht worden). Gefunden über opel-hbl.de, das genau diese Adressen aufruft. Je Wettbewerb werden **beide** Endpunkte gebraucht (#231): `fixtures` liefert den vollen Spielplan, in der Liga aber ausschließlich Angesetztes (297 Spiele, 0 Ergebnisse) – beim Pokal umgekehrt, dort stehen die Ergebnisse drin; `fixtures_ribbon` liefert den aktuellen Spieltag und **nur dort stehen die Liga-Ergebnisse**, etwa von einem Tag vor bis drei Tage nach dem Spiel. Beim Zusammenführen gewinnt die Fassung MIT Ergebnis, sonst überschreibt der Spielplan ein eingesammeltes Resultat wieder mit NULL. Ein `round`-Parameter an der Liste wird angenommen, aber ignoriert – **aber `fixtures?locale=de-DE&state=<Widget-Zustand>` liefert den „Ergebnisse"-Reiter: alle gespielten Spiele der Saison mit Endstand und Spieltag** (Nachtrag #263, 07.09.2026; der Zustand steht in `data.subPageTabs[RESULTS].link` hinter `fl~`, oder selbst gebaut: zlib+base64 von `{"l":"de-DE","s":<seasonId>,"z":"RESULTS"}`, `_sr_ergebnis_zustand()`). `_profi_spiele()` fragt je Embed drei Listen: Spielplan, Ergebnisse, Ribbon. Dazu **der Einzelspiel-Endpunkt `fixture_detail?locale=de-DE&fixtureId=<id>` (#263)**: liefert zu jeder Kennung Status (`CONFIRMED`) und Endstand (`competitors[].score` als Text, Halbzeiten in `periodData`), egal wie alt das Spiel ist; `_profi_nachladen()` zieht damit vergangene Spiele ohne bestätigten Endstand nach (max. 6 je Seitenaufruf, je Spiel höchstens stündlich). `standings` nur bei 248. **Zeiten (#264): `startTimeUTC` ist UTC ohne Zeitzone, `date` im Ribbon dagegen Ortszeit** – `startTimeUTC` wird bevorzugt. Während des Spiels trägt der Ribbon Zwischenstände mit `isLive=true`; Endstand ist nur, was `isFinal` bzw. Status `CONFIRMED` trägt (`tvb_spiele.bestaetigt`). Zwei Formen (`fixture`-Objekt im Ribbon, flach in `fixtures`) |
...
teile/
  ...
  18_tvb.py           – /a/tvb/<token>/ Naechste Spiele, Ergebnisse und
                       ...
                       (Wunsch #120). Seit #263/#264 (06.09.2026): _sr_spiel()
                       kennt drei Zustaende - Ended nur bei isFinal/CONFIRMED
                       (bestaetigt=1), Live bei isLive (Zwischenstand, rot mit
                       "laeuft" in der Vorlage), sonst Pre; startTimeUTC vor
                       `date` (Ortszeit!). _sr_spiel_aus_detail() liest den
                       Einzelspiel-Endpunkt fixture_detail, _profi_nachladen()
                       zieht damit bei jedem Seitenaufruf der Profis bis zu 6
                       vergangene Spiele ohne bestaetigten Endstand nach
                       (Pokal zuerst im Embed 255; je Spiel hoechstens ein
                       Versuch je Stunde ueber aktualisiert_am). Der UPSERT
...
- `test_tvb_nachladen.py` – Wunsch #263/#264. Der Hamburg-Fall als Test:
  ...
  Einzelspiel-Endpunkt (Tore als Text, Status als Zeichenkette),
  `_profi_nachladen()` (33:31 wird 34:34 samt richtigem Anwurf, bestaetigte
  Spiele werden nicht angefragt, unbekannte hoechstens stuendlich, Pokal
  zuerst im Pokal-Embed, laufende Spiele nicht als Endstand, Zukunft und
  handball.net-Spiele aussen vor, Obergrenze je Aufruf) und der UPSERT
  (bestaetigt ist unantastbar, Anwurf kommt trotzdem an, NULL ueberschreibt
  keinen Wettbewerb). Die Seite zeigt ein laufendes Spiel mit "laeuft" und
  ohne Sieger.
- `test_tvb_datenschicht.py` – Neubau nach dem Relaunch (#192/#193), dazu
  #231/#232. Der wichtigste Test der Datei ist
  `test_ergebnis_wird_nicht_vom_spielplan_ueberschrieben`: dasselbe Spiel kommt
  aus BEIDEN Sportradar-Endpunkten, einmal mit Ergebnis (Ribbon), einmal ohne
  (Spielplan) - gewinnt der Spielplan, ist ein gerade eingesammeltes Resultat
  sofort wieder weg, und der Fehler traete nur auf, wenn beide Endpunkte
  dasselbe Spiel fuehren. Dazu `test_pokalspiel_kommt_aus_dem_zweiten_embed`,
  weil der DHB-Pokal ein eigenes Embed hat und #151 sonst beim Neubau lautlos
  rueckgaengig gemacht worden waere. Loest
